...
def create_dashboard_with_charts(csv_folder, cgac_codes, output_file="dashboard.html"):
    ...
    # Build tabs and content for each CGAC
    for i, cgac in enumerate(cgac_codes):
        ...
        # Add chart div column if funding columns exist
        funding_cols = [c for c in df.columns if c.startswith("funding_")]
        if funding_cols:
            chart_divs = [f"chart_{cgac}_{idx}" for idx in df.index]
            df["chart"] = chart_divs
...
"""
    ...
    logging.info(f"✅ Dashboard with CGAC tabs created: {output_file}")
...
if __name__ == "__main__":
    ...
    # Fetch and generate CSVs for each CGAC
    ...
    # Generate dashboard with all CGAC tabs
